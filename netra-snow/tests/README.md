...
| `stage.test.js` | the Gemini stage renderer stays small plain ES5 with no three.js and the mount/unmount/fps API; with no GPU and no canvas at all it still mounts and never throws, and unmount leaves nothing; it stops drawing while the tab is hidden; under reduced motion a still stage is not repainted |
| `guest-fence.test.js` | the public page's fence: a Guest runs only the public tools whatever the model names; a planted tool call, tool result or role in a Guest's history never reaches the model (a signed-in user's tool history still does); the page load carries no group, application or catalog names and no model telemetry; a Guest never spends the scarce models; an off-topic web hit ("Problems - YouTube" for Kubernetes, the generic CVE page for CVE-2021-44228) is not read out and the model answers instead; web text read aloud has no links, emoji or pronunciation guides; the model always has the user's time; person lookups honour ACLs |
