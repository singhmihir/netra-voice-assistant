...
T.test('a colleague\'s nudge is heard in the debrief, and can not be "undone" as some other task', function () {
    ...
    T.match(u.message, /nothing to undo/);
...
T.test('brain stops after a create: the partial answer says what was raised and does not invite a repeat', function () {
    ...
    T.match(r.message, /Those changes are done/);
});

T.test('a write after six reads is still reported', function () {
    var s = new S.Session();
    var reads = [];
    for (var i = 0; i < 6; i++) reads.push(['get_ticket_status', { ticket_number: 'INC00100' + (13 + i % 6) }]);
    ...
    var r = s.say('check all my tickets and tell 13 we are rebooting');
    T.match(r.message, /Comment added to INC0010013/);
    T.match(r.message, /incident ending 0 1 3\*\* is in progress/, 'status reads say the state, not "I ran"');
