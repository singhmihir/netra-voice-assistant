/**
 * Netra - v3.0 master update-set capture (idempotent-ish; safe to re-run,
 ...
 *
 * What it does:
 *   1. creates the parent update set "Netra - v3.0" plus six children
...
(function () {
    ...
    var VERSION    = 'Netra - v3.0';

    var app = new GlideRecord('sys_app');
    if (!app.get('scope', SCOPE_NAME)) {
        gs.error('[netra-v3-capture] scoped app not found: ' + SCOPE_NAME);
        return;
    }
    ...
    function capture(gr, childKey) {
        try {
            ...
            gs.warn('[netra-v3-capture] failed ' + gr.getRecordClassName() + ':' + gr.getUniqueValue() + ' - ' + e);
        }
    }
    ...
    gs.info('[netra-v3-capture] DONE parent=' + parentId + ' metadata=' + seen + ' by-child: ' + totals.join(', '));
})();
