/**
 * Netra - v4.0 master update-set capture (idempotent-ish; safe to re-run,
 ...
 *
 * What it does:
 *   1. creates the parent update set "Netra - v4.0" plus six children
...
(function () {
    ...
    var VERSION    = 'Netra - v4.0';

    var app = new GlideRecord('sys_app');
    if (!app.get('scope', SCOPE_NAME)) {
        gs.error('[netra-v4-capture] scoped app not found: ' + SCOPE_NAME);
        return;
    }
    ...
    function capture(gr, childKey) {
        try {
            ...
            gs.warn('[netra-v4-capture] failed ' + gr.getRecordClassName() + ':' + gr.getUniqueValue() + ' - ' + e);
        }
    }
    ...
    gs.info('[netra-v4-capture] DONE parent=' + parentId + ' metadata=' + seen + ' by-child: ' + totals.join(', '));
})();
