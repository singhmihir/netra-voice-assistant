/**
 * Netra - v6.0 master update-set capture (idempotent-ish; safe to re-run,
 ...
 *
 * What it does:
 *   1. creates the parent update set "Netra - v6.0" plus six children
...
(function () {
    ...
    var VERSION    = 'Netra - v6.0';

    var app = new GlideRecord('sys_app');
    if (!app.get('scope', SCOPE_NAME)) {
        gs.error('[netra-v6-capture] scoped app not found: ' + SCOPE_NAME);
        return;
    }
    ...
    function capture(gr, childKey) {
        try {
            ...
            gs.warn('[netra-v6-capture] failed ' + gr.getRecordClassName() + ':' + gr.getUniqueValue() + ' - ' + e);
        }
    }
    ...
    gs.info('[netra-v6-capture] DONE parent=' + parentId + ' metadata=' + seen + ' by-child: ' + totals.join(', '));
})();
