 ...
 * Canvas2D fallback without WebGL. Reads __netraState/__netraLevel/__netraBands, plus
 * __netraMode (muted / paused / ended: a look of its own) and __netraCalm (Calm visuals).
 * The orb draws at most ~30 fps.
 * API: window.NetraStage3D = { mount(host) -> bool, unmount(), fps() -> number, peek(st) }. ES5.
 */
window.NetraStage3D = (function () {
    'use strict';

    var root = null, O = null, G = null, host = null, sprites = null;
    var raf = 0, mq = null, mqReduced = false, reduced = false, onVis = null, onMq = null, onResize = null;
    ...
    function targets(st) {
        ...
        // the page's mode wins: mic off is grey and still with no glow, paused a dim
        // breath, ended dark - each state has a look of its own
        var m = window.__netraMode;
        if (m === 'muted') { T.spd = 0; T.dim = 1; T.gain = 0.32; T.glowG = 0; T.glowH = 0.06; T.talk = 0; T.voice = 0; T.think = 0; T.warm = 0; T.cool = 0; }
        else if (m === 'paused') { T.spd = 0.25; T.dim = 0.7; T.gain = 0.5; T.glowG = 0.12; T.glowH = 0.08; }
        else if (m === 'ended') { T.spd = 0; T.dim = 1; T.gain = 0.25; T.glowG = 0; T.talk = 0; T.voice = 0; T.think = 0; }
    }
    // the foot glow: rises with your voice while listening (talk) and with hers while
    // she speaks (voice); a slow ~0.23 Hz swell while she works (well under 3 Hz)
    function glowHeight() {
        return P.glowH + (0.16 * P.talk + 0.08 * P.voice) * lvS + 0.07 * P.think * (0.5 + 0.5 * Math.sin(swPh * 0.55));
    }
    // breathing stops when the orb holds still (mic off, ended)
    function breath() { return reduced ? 0 : 0.022 * (1 - 0.5 * P.dim) * Math.min(1, P.spd * 4) * Math.sin(brPh); }
    ...
    function measure(now) {
        measAt = now;
        // Calm visuals (the page's setting) or the system's reduced motion: a composed still
        var calm = mqReduced || !!window.__netraCalm;
        if (calm !== reduced) { reduced = calm; if (O) O.sig = -1; }
        var hr = host.getBoundingClientRect(), wrap = document.querySelector('.netra-stage-blob-wrap'), r = wrap && wrap.getBoundingClientRect(), box, R, half, s, x, y, gone;
        ...
        // the page gave the orb's row away (the keyboard is open on a small phone):
        // no orb, rather than a big one painted over the captions at the centre
        gone = r && !(r.width > 10) && wrap.getClientRects && wrap.getClientRects().length ? 'hidden' : '';
        if (s.visibility !== gone) s.visibility = gone;
    ...
    function drawOrb(swell) {
        ...
        gl.uniform4f(U.uA, tFlow, shPh, 1 + breath(), 0.14 * Math.sin(tFlow * 0.31));
    ...
    function drawGlow() {
        ...
        gl.uniform4f(U.uG, glowHeight(), P.glowG + 0.34 * lift, P.cool + 0.6 * lvS * P.talk, P.warm * P.voice + 0.25 * P.think);
    ...
    function orb2D(swell) {
        var c2 = O.c2, w = O.w, t = tFlow, cx = w / 2, cy = cx, R = w / (2 * OK) * (1 + swell) * (1 + breath());
    ...
    function glow2D() {
        ...
        var gh = glowHeight() / GH * h * 3.3, ga = (P.glowG + 0.34 * lift) * 0.5 * (1 - 0.6 * P.dim);
    ...
    function tick(ts) {
        ...
        if (!reduced) {
            ...
            frameNo++;
            // ~30 fps at most for the orb, whatever the display's refresh rate
            dO = frameNo % i === 0 && ts - lastDraw >= 32;
            dG = frameNo % k === 1;
        } else {
    ...
    function mount(el) {
        ...
        try {
            host = el; css();
            mqReduced = !!(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
            reduced = mqReduced || !!window.__netraCalm;
            ...
            if (window.matchMedia) {
                ...
                onMq = function () { mqReduced = mq.matches; reduced = mqReduced || !!window.__netraCalm; O.sig = -1; };
    ...
    // tests: the targets a state (and the page's __netraMode) asks for
    function peek(st) { targets(st); return { spd: T.spd, glowG: T.glowG, glowH: T.glowH, dim: T.dim, think: T.think }; }

    return { mount: mount, unmount: unmount, fps: fps, peek: peek };
})();
