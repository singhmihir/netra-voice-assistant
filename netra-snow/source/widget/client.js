...
api.controller = function ($scope, $timeout, $window) {
    ...
    function _previewVoice() {
        ...
        if (eng === 'netra' && (_netraVoiceReady() || _netraVoiceComing())) who = VOICE_NAME;   // said by her, now or once she is there
    ...
    function _destroyController() {
        ...
        try { _netraVoiceStop(); } catch (e) {}   // v7.9 - her voice worker goes with the page
    ...
    function tryBoot(fromTap) {
        ...
        try {
            ...
            _readyUpdate();
            if (!_isPhone()) $timeout(function () { if (!_nativeHeardWords && !_ctrlDestroyed) _afterVoice(function () { _earLoad(true); }, 60000, true); }, 15000);
    ...
    var VOICE_WORKER_SRC =
        "let pdiBase = '';\n" +
        // every file from this instance goes through the browser's Cache
        // storage: the second visit needs no network at all
        "let store = null;\n" +
        "async function cachedFetch(url, opts) {\n" +
        "  try { if (!store && self.caches) store = await caches.open('netra-voice-v1'); } catch (e) { store = null; }\n" +
        "  if (store) { try { const hit = await store.match(url); if (hit) return hit; } catch (e) {} }\n" +
        "  const res = await realFetch(url, opts);\n" +
        "  if (store && res && res.ok) { try { store.put(url, res.clone()).catch(() => {}); } catch (e) {} }\n" +
        "  return res;\n" +
        "}\n" +
        // the runtime fetches its .wasm by name: the suffix the platform eats
        "const realFetch = self.fetch ? self.fetch.bind(self) : null;\n" +
        "if (realFetch) self.fetch = (url, opts) => { let u = String(url && url.url || url); if (pdiBase && u.indexOf(pdiBase) === 0) { u = u.replace(/\\.(json|js|wasm)$/, '-$1'); return cachedFetch(u, opts); } return realFetch(u, opts); };\n" +
        ...
        "};\n" +
        // a body read in pieces, the bytes so far told to the page as they land
        "let gotBytes = 0, toldAt = 0;\n" +
        "async function readAll(res) {\n" +
        "  if (!res || !res.ok) throw new Error('HTTP ' + (res && res.status));\n" +
        "  if (!res.body || !res.body.getReader) { const b = await res.arrayBuffer(); gotBytes += b.byteLength; return b; }\n" +
        "  const reader = res.body.getReader(), parts = []; let n = 0;\n" +
        "  for (;;) { const { done, value } = await reader.read(); if (done) break; parts.push(value); n += value.byteLength; gotBytes += value.byteLength;\n" +
        "    const now = Date.now(); if (now - toldAt > 500) { toldAt = now; self.postMessage({ stage: 'fetching', mb: Math.round(gotBytes / 1048576) }); } }\n" +
        "  const out = new Uint8Array(n); let o = 0; for (const p of parts) { out.set(p, o); o += p.byteLength; } return out.buffer;\n" +
        "}\n" +
        ...
        "  try { await say({ id: 0, text: 'Ready.', warm: true }); } catch (e) {}\n" +
        ...
        // (5 s a try, two tries a sentence, twice: under the page's 30 s stall watch, so a bad sentence is skipped, not a restart)
        "async function phonemize(text) {\n" +
        "  const run = () => new Promise((resolve, reject) => { const t = setTimeout(() => reject(new Error('phonemizer silent')), 5000); gotIds = (ids) => { clearTimeout(t); resolve(ids); }; try { phon.callMain(['-l', cfg.espeak.voice, '--input', JSON.stringify([{ text }]), '--espeak_data', '/espeak-ng-data']); } catch (e) { clearTimeout(t); reject(e); } });\n" +
        "  try { return await run(); } catch (e) { phon = await makePhon(); return await run(); }\n" +
        "}\n" +
        // a sentence that fails is tried once more, then reported (the page skips it: never another voice)
        "async function sayTwice(d) { try { await say(d); } catch (e) { await say(d); } }\n" +
        "async function say(d) {\n" +
        "  const ids = await phonemize(String(d.text || ''));\n" +
        "  if (dropped(d.id) && !d.warm) { self.postMessage({ id: d.id, dropped: true }); return; }\n" +
        "  if (!ids || !ids.length) { if (!d.warm) self.postMessage({ id: d.id, pcm: new Float32Array(0), rate: cfg.audio.sample_rate, ms: 0 }); return; }\n" +
        ...
        "}\n";
    c.voice = { status: 'off', error: '', loadMs: 0, said: 0, stage: '', mb: 0 };
    ...
    var _voiceOutLen = {};   // the length of each sentence the worker still owes, kept even after the page drops the job
    ...
    var VOICE_LOAD_STALL_MS = 45000;    // a load that says nothing for this long (no bytes, no step) is dead: started again
    var VOICE_STALL_MS = 30000;         // a sentence the worker gives no answer to in this long means it is stuck
    // a short thinking cue or two in her own voice, made once she is ready (played from the filler cache)
    var VOICE_FILLERS = ['One moment, please.', 'Checking on that now.', 'Let me pull that up.', 'Just a second.'];
    function _voiceState() { if (!c.voice) c.voice = { status: 'off', error: '', loadMs: 0, said: 0, stage: '', mb: 0 }; return c.voice; }
    ...
    // still coming: her first load within its window, or a restart (any time in the visit) within its shorter one
    ...
    function _netraVoiceWaitOver() {
        _voiceWaitTimer = null;
        if (_netraVoiceReady() || _netraVoiceComing()) return;
        if (_voiceHeld.length || !_voiceEverReady) logEvent('warn', 'my own voice is not ' + (_voiceEverReady ? 'back' : 'ready') + ' after ' + Math.round((_voiceRestartAt ? VOICE_RESTART_WAIT_MS : VOICE_WAIT_MS) / 1000) + ' s - this device\'s voice until it is');
        _netraVoiceFlush();
        _readyUpdate();
    }
    // work that should wait for her voice (the ear's download competes with hers): fn once she is ready, or after capMs
    function _afterVoice(fn, capMs, wholeLoad) {
        var t0 = Date.now();
        (function tick() {
            if (_ctrlDestroyed) return;
            var v = _voiceState(), busy = v.status === 'loading' && (wholeLoad || v.stage !== 'starting');
            if (!busy || Date.now() - t0 >= capMs) { fn(); return; }
            $timeout(tick, 400, false);
        })();
    }
    function _netraVoiceLoad() {
        var vs = _voiceState();
        if (_voiceWorker || vs.status === 'loading' || !_netraVoiceWanted() || _ctrlDestroyed) return;
        vs.status = 'loading'; vs.error = ''; vs.stage = ''; vs.mb = 0; _voiceLoadStart = _voiceLastMsg = Date.now();
        if (!_voiceFirstStart) { _voiceFirstStart = _voiceLoadStart; _voiceWaitTimer = $timeout(_netraVoiceWaitOver, VOICE_WAIT_MS, false); }
        try {
    ...
    function _netraVoiceLoadWatch() {
        if (_voiceLoadTimer) { $timeout.cancel(_voiceLoadTimer); _voiceLoadTimer = null; }
        if (_voiceState().status !== 'loading' || !_voiceWorker) return;
        var left = VOICE_LOAD_STALL_MS - (Date.now() - _voiceLastMsg);
        if (left <= 0) { _netraVoiceFail('no progress for ' + Math.round(VOICE_LOAD_STALL_MS / 1000) + ' s while loading' + (c.voice.stage ? ' (' + c.voice.stage + ')' : '')); return; }
        _voiceLoadTimer = $timeout(function () { _voiceLoadTimer = null; _netraVoiceLoadWatch(); }, Math.max(1000, left), false);
    }
    function _netraVoiceOnMessage(ev) {
        var d = ev.data || {};
        _voiceLastMsg = Date.now();
        if (d.stage) {
            c.voice.stage = d.stage; if (d.mb !== undefined) c.voice.mb = d.mb;
            if (d.stage === 'starting' && _voiceLoadTimer) _netraVoiceLoadWatch();   // the watch is re-read from this step
            _readyUpdate(); return;
        }
        if (d.ready) {
            ...
            _netraVoiceFlush();
            $timeout(_netraFillersPrepare, 6000, false);
            return;
        }
        if (d.id !== undefined) {
            var at = _voiceOut.indexOf(d.id); delete _voiceOutLen[d.id];
            if (at >= 0) { _voiceOut.splice(at, 1); if (at === 0) _voiceHeadSince = Date.now(); }
            var job = _voiceJobs[d.id]; delete _voiceJobs[d.id];
            _netraVoiceWatch();
            if (!job) return;
            if (d.dropped) { job.cb('skip'); return; }
            // a sentence she could not make (tried twice) or that has no sound in it is left out, never said in another voice
            if (d.error) { logEvent('warn', 'my own voice: ' + d.error + ' - "' + job.text.substring(0, 40) + '" left out'); job.cb('skip'); return; }
            if (!d.pcm || !d.pcm.length) { job.cb('skip'); return; }
            c.voice.said++;
    ...
    function _netraVoiceFail(msg, fatal) {
        ...
        _voiceWorker = null; _voiceOut = []; _voiceOutLen = {};
        if (_voiceLoadTimer) { $timeout.cancel(_voiceLoadTimer); _voiceLoadTimer = null; }
        ...
        else _netraVoiceFlush();
        _readyUpdate();
    }
    // the page goes: the worker with it, and nothing waits for her any more
    function _netraVoiceStop() {
        try { if (_voiceWorker) _voiceWorker.terminate(); } catch (e) {}
        _voiceWorker = null; _voiceOut = []; _voiceOutLen = {};
        if (_voiceStallTimer) { $timeout.cancel(_voiceStallTimer); _voiceStallTimer = null; }
        if (_voiceLoadTimer) { $timeout.cancel(_voiceLoadTimer); _voiceLoadTimer = null; }
        if (_voiceWaitTimer) { $timeout.cancel(_voiceWaitTimer); _voiceWaitTimer = null; }
        _voiceHeld = []; _voiceRestartAt = 0;
        if (_voiceJobs) for (var k in _voiceJobs) delete _voiceJobs[k];
        if (c.voice) { c.voice.status = 'off'; c.voice.stage = ''; }
    }
    // lines that arrived while she was loading: said now, in order, by whatever voice there is
    function _netraVoiceFlush() {
        var held = _voiceHeld; _voiceHeld = [];
        if (!held.length) return;
        var i = 0;
        (function next() { if (i >= held.length) return; var h = held[i++]; speak(h.text, function () { if (h.done) h.done(); next(); }); })();
    }
    // her thinking cues, from the filler cache (the device's speech synthesis is never used for them)
    function _netraFillersPrepare() {
        if (!_netraVoiceReady() || _fillersPrepared) return;
        _fillersPrepared = true;
        var i = 0, tries = 0;
        (function one() {
            if (i >= VOICE_FILLERS.length || tries > 12) return;
            if (!_netraVoiceReady()) { _fillersPrepared = false; return; }   // made once she is back
            var phrase = VOICE_FILLERS[i]; tries++;
            var onMade = function (blob) {
                if (blob && blob !== 'skip') { i++; try { fillerCache.push({ url: URL.createObjectURL(blob), text: phrase, voice: 'netra' }); } catch (e) {} }
                if (i >= VOICE_FILLERS.length) { logEvent('tts', 'thinking cues in my own voice: ' + fillerCache.filter(function (x) { return x.voice === 'netra'; }).length); return; }
                $timeout(one, blob && blob !== 'skip' ? 300 : 3000, false);   // a dropped or failed cue is tried again a little later
            };
            onMade.filler = true;
            _netraVoiceSynth(phrase, onMade);
        })();
    }
    var _fillersPrepared = false;
    // a barge-in, a stop or a new line: what is still queued in the worker is not made
    function _netraVoiceDrop() {
        var upTo = 0;
        if (!_voiceJobs) return;
        for (var k in _voiceJobs) {
            var j = _voiceJobs[k];
            if (j.filler) continue;   // a thinking cue being made is left to finish
            delete _voiceJobs[k]; upTo = Math.max(upTo, Number(k)); j.cb('skip');
        }
        _netraVoiceWatch();
        if (upTo && _voiceWorker) { try { _voiceWorker.postMessage({ cmd: 'drop', upTo: upTo, keep: Object.keys(_voiceJobs).map(Number) }); } catch (e) {} }
    }
    // one sentence group -> a WAV blob (null when she can not)
    function _netraVoiceSynth(text, cb) {
        if (!_netraVoiceReady()) { cb(null); return; }
        var id = ++_voiceJobId, job = { cb: cb, text: String(text), filler: !!(cb && cb.filler) };
        _voiceJobs[id] = job;
        if (!_voiceOut.length) _voiceHeadSince = Date.now();
        _voiceOut.push(id); _voiceOutLen[id] = String(text).length;
        _netraVoiceWatch();
        var rate = parseFloat(c.speechRate) || 1;
        _voiceWorker.postMessage({ cmd: 'say', id: id, text: String(text), lengthScale: 1 / rate });
    }
    // one watch on the sentence the worker is on (the oldest it owes an answer to): 30 s with no answer means it
    // is stuck. New lines and drops do not move it - a live worker answers a dropped sentence at once, a stuck one never
    var _voiceStallTimer = null;
    function _netraVoiceWatch() {
        ...
        var head = _voiceJobs && _voiceJobs[_voiceOut[0]], allow = VOICE_STALL_MS + (_voiceOutLen[_voiceOut[0]] || (head ? head.text.length : 0)) * 100;
    ...
    c.ear = { mode: 'auto', size: 'small', on: false, status: 'off', progress: 0, loadedMb: 0, sizeUnknown: false, src: '', prepared: false, model: EAR_MODEL_TINY, device: 'wasm', error: '', heard: 0, why: '', lastMs: 0 };
    ...
    function _voiceReady() {
        ...
        if (eng === 'netra') {
            ...
            if (_netraVoiceComing()) { c.gate.voiceText = (c.voice.stage === 'starting' ? 'starting my own voice…' : 'loading my own voice from this instance…' + (c.voice.mb ? ' ' + c.voice.mb + ' MB' : '')); return false; }
        }
    ...
    function _plainGateText(raw) {
        ...
        s = s.replace(/loading my on-device ear( from this instance)?( \d+%)?( \([^()]*, once\))?/, function (m, here) { return (here ? 'getting speech recognition ready from this instance, one time' : 'downloading speech recognition, one time') + (note ? ' (' + note + ')' : ''); })
             ...
             .replace(/^loading my own voice from this instance…( \d+ MB)?$/, 'Getting her voice ready from this instance…$1')
             .replace(/^starting my own voice…$/, 'Starting her voice…')
    ...
    function _earSizeNote() { return _earSizeName() + ', about ' + _earSizeMb() + ' MB' + (_earSlowHere() ? ', the most accurate and slower on this device' : '') + (c.ear.sizeUnknown && c.ear.loadedMb ? ', ' + c.ear.loadedMb + ' MB so far' : ''); }
    ...
    var EAR_WORKER_SRC =
        ...
        // a server that sends no size: the library reports total = loaded on
        // every event, so a file is of known size only once loaded < total
        "          const f = files[p.file] || [0, 0, false]; f[0] = p.loaded || 0; f[1] = p.total || 0; f[2] = f[2] || (f[0] < f[1]); files[p.file] = f;\n" +
        "          let got = 0, all = 0, known = true;\n" +
        "          for (const k in files) { got += files[k][0]; all += files[k][1]; if (!files[k][2]) known = false; }\n" +
...
"      const r = await asr(e.data.audio);\n" +
    ...
    function _earSummary() {
        var e = c.ear;
        if (e.status === 'loading') return 'on-device: loading Whisper ' + (e.sizeUnknown && e.loadedMb ? e.loadedMb + ' MB so far' : e.progress + '%');
    ...
    function _earStart(why, quiet) {
        ...
        try {
            if (!_earWorker) {
                c.ear.src = '';
                _earPickModel();
                // v7.9 - her voice's files first: the ear's download waits for her (up to 12 s)
                _afterVoice(function () { if (!_earWorker && c.ear.status === 'loading') { try { _earSpawn(); } catch (eS) { _earFail(String(eS && eS.message || eS)); } } }, 45000);
            } else {
                _earReady();
            }
        } catch (e) { _earFail(String(e && e.message || e)); }
    ...
    function _earSpawn() {
        c.ear.progress = 0; c.ear.prepared = false; c.ear.loadedMb = 0; c.ear.sizeUnknown = false;   // a new download counts from zero once
    ...
    // frozen for good (the model site blocked by a proxy, a GPU compile that
    // hangs). No word from the worker for EAR_STALL_MS while loading: the
    // GPU model gives way to the smaller one on WebAssembly, and that one
    // gives up with a reason, so the card offers typing instead
    var EAR_STALL_MS = 45000, _earLastMsgAt = 0, _earWatchTimer = null;
    function _earWatchArm() {
        _earLastMsgAt = Date.now();
        try { if (_earWatchTimer) $window.clearTimeout(_earWatchTimer); } catch (e0) {}
        _earWatchTimer = null;
        try { if ($window.setTimeout) _earWatchTimer = $window.setTimeout(_earStallCheck, 5000); } catch (e1) {}
    }
    function _earStallCheck() {
        _earWatchTimer = null;
        if (!_earWorker || c.ear.status !== 'loading') return;
        var quiet = Date.now() - _earLastMsgAt;
        if (quiet < EAR_STALL_MS) { try { if ($window.setTimeout) _earWatchTimer = $window.setTimeout(_earStallCheck, 5000); } catch (e1) {} return; }
        var what = c.ear.loadedMb || c.ear.progress ? 'the download stopped' : 'the download did not start';
        logEvent('warn', 'on-device ear: ' + what + ' (' + Math.round(quiet / 1000) + ' s without a word from the worker)');
        _earLoadFailed(what + ' - the model site may be blocked on this network');
    }
    ...
    function _earOnMessage(ev) {
        var d = ev.data || {};
        _earLastMsgAt = Date.now();
        if (d.mb !== undefined && c.ear.status === 'loading' && !c.ear.prepared) {
            c.ear.loadedMb = Math.max(c.ear.loadedMb || 0, Math.floor(Number(d.mb) || 0));
            if (d.progress === undefined) {
                // no size from the server (this instance sends its files chunked): the
                // bar runs against the size expected for this model, the card says the MB
                c.ear.sizeUnknown = true;
                var est = _earSizeMb(), pctEst = est ? Math.min(99, Math.floor(c.ear.loadedMb / est * 100)) : 0;
                if (pctEst > c.ear.progress) c.ear.progress = pctEst;
                _readyUpdate();
            } else c.ear.sizeUnknown = false;
        }
    ...
    function _silenceCurrentAudio() {
        ...
        // v7.9 - a sentence of hers cut short: its blob URL is let go (ended never fires for a paused clip)
        if (currentAudio._netraUrl) { try { URL.revokeObjectURL(currentAudio._netraUrl); } catch (e) {} currentAudio._netraUrl = null; }
        currentAudio = null;
    }
    ...
    function stopSpeaking(reason) {
        ...
        _voiceHeld = [];                        // v7.9 - a line that waited for her was stopped too
    ...
    function speak(text, done) {
        ...
        // v7.9 - her own voice is still coming: the line waits for it, untouched
        // (nothing marked as speaking, no session bumped, the caption when she says it)
        if ((c.ttsEngine || 'browser') === 'netra' && !c.labMute && !_netraVoiceReady() && _netraVoiceComing()) {
            _voiceHeld.push({ text: text, done: done });
            if (!_voiceWaitSaid) { _voiceWaitSaid = true; logEvent('tts', 'my own voice is still ' + (_voiceState().status === 'error' ? 'restarting' : 'loading') + ' - this line waits for it'); }
            return;
        }
        ...
        if (engine === 'netra' && !_netraVoiceReady()) {
            if (!_voiceWaitSaid) { _voiceWaitSaid = true; logEvent('tts', 'my own voice is ' + (_voiceState().status === 'off' ? 'not on this instance' : 'not available') + ' - this device\'s voice instead'); }
            engine = 'browser';
        }
    ...
    function speakNetraVoice(text, done) {
        ...
        function playNext() {
            ...
            var ready = function () {
                if (session !== _speakSessionId) return;
                if (blobs[i] === 'skip') return playNext();   // dropped, or a sentence she could not make: left out
                if (blobs[i] === false) {
                ...
                audio._netraUrl = url;   // let go by _silenceCurrentAudio if this clip is cut
                ...
                var advanceOnce = function () {
                    ...
                    if (audio._netraUrl) { URL.revokeObjectURL(url); audio._netraUrl = null; }
    ...
    function _playOneFiller(onEndedExtra) {
        ...
        // v7.9 - on her engine only the cues made in her voice; on the neural engine only the neural ones
        var mine = (c.ttsEngine || 'browser') === 'netra';
        var ownCues = fillerCache.filter(function (x) { return mine ? x.voice === 'netra' : x.voice !== 'netra'; });
        if (ownCues.length) {
            ...
            var pool = ownCues.filter(function (x) { return vr || !VR_FILLER_RE.test(x.text); });
            if (!pool.length) pool = ownCues;
        ...
        // v7.9 - on her own engine a cue is one of hers or nothing: a short
        // pause keeps the chain's pacing, and the device's synthesis stays silent
        if ((c.ttsEngine || 'browser') === 'netra') {
            $timeout(function () { if (onEndedExtra) onEndedExtra(); }, 900, false);
            return;
        }
    ...
    // v7.9 - her own voice starts loading the moment the page has its data,
    // not when the user's tap boots her: a few seconds sooner on every visit
    $timeout(function () { if (!_ctrlDestroyed) _netraVoiceLoad(); }, 0, false);
};
