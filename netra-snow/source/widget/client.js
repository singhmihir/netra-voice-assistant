...
api.controller = function ($scope, $timeout, $window) {
    ...
    function _firstRunCheck() {
        ...
        if (!firstEver && recentGood && micOk) {
            logEvent('lab', 'mic check skipped - scored ' + saved.score + '% recently; say "mic check" to run it again');
            c.labCalib = { stage: 'saved', heard: '', score: saved.score, verdict: 'last run ' + String(saved.at).substring(0, 10) + ' - say "mic check" to run again' };
            speak('I am listening - just speak.');
            $scope.$applyAsync();
            return;
        }
    ...
    c.labSetEar = function () {
        ...
        else if (c.ear.mode === 'off') _earStop('switched off in the Lab');
        else if (c.ear.on && !c.ear.why) _earStop('back to automatic');
    };
    function _earStart(why, quiet) {
        if (c.ear.mode === 'off') { logEvent('rec', 'ear wanted (' + why + ') but switched off in the Lab'); return false; }
        if (c.ear.on || c.ear.status === 'loading') return true;
        if (typeof Worker === 'undefined' || typeof Blob === 'undefined') { c.ear.status = 'error'; c.ear.error = 'no workers in this browser'; return false; }
        c.ear.why = why; c.ear.status = 'loading'; c.ear.progress = 0; c.ear.error = ''; _earNativeSeen = 0;
        logEvent('rec', 'on-device ear starting: ' + why);
        if (!quiet && !_earSaid) {
            _earSaid = true;
            speak('I can hear you, but the browser is returning no words for what you say. I am switching to listening on this device myself - give me a few seconds to load it.');
        }
        try {
            if (!_earWorker) {
    ...
    function _earReady() {
        c.ear.on = true; c.ear.status = 'on'; c.ear.progress = 100;
        _earTapAttach();
        logEvent('rec', 'on-device ear listening (' + c.ear.model + ' on ' + c.ear.device + ')');
        cue('wake');
        if (c.ear.why && !/Lab/.test(c.ear.why)) speak('Ready - I am listening on this device now. Just speak.');
        $scope.$applyAsync();
    }
    ...
    function _earStop(why) {
        ...
        c.ear.on = false; c.ear.status = 'off'; c.ear.why = '';
        _earSeg = []; _earSegMs = 0; _earInSpeech = false; _earQueue = [];
        logEvent('rec', 'on-device ear off: ' + why);
        $scope.$applyAsync();
    }
    ...
    function startContinuous() {
        if (!_ctrlDestroyed && !c.ear.on && c.ear.status !== 'loading') {
            ...
            if (ev.error === 'network') {
                ...
                if (_netErrStreak === 3) {
                    ...
                    if (!_earStart('the browser can not reach its speech service') && Date.now() - _lastNetErrSaidAt > 300000 && !_speakingNow) {
                        ...
                        speak('I can hear sound, but the browser can not reach its speech service right now, so I can not make out words. That is usually the network or a VPN. You can still type to me in the Lab.');
                    }
                    $scope.$applyAsync();
                }
                return;
    ...
    var NETRA_BUILD = 'v7.2-voice';   // bumped: reopens Edge TTS for everyone whose breaker tripped on an old build
    try {
