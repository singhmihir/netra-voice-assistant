...
api.controller = function ($scope, $timeout, $window) {
    ...
    function _earSizeNote() { return _earSizeName() + ', about ' + _earSizeMb() + ' MB' + (_earSlowHere() ? ', the most accurate and slower on this device' : '') + (!c.ear.progress && c.ear.loadedMb ? ', ' + c.ear.loadedMb + ' MB so far' : ''); }
...
"      const r = await asr(e.data.audio);\n" +
    ...
    function _earSummary() {
        var e = c.ear;
        if (e.status === 'loading') return 'on-device: loading Whisper ' + (e.progress || !e.loadedMb ? e.progress + '%' : e.loadedMb + ' MB so far');
    ...
    function _earOnMessage(ev) {
        var d = ev.data || {};
        _earLastMsgAt = Date.now();
        if (d.mb !== undefined && c.ear.status === 'loading' && !c.ear.prepared) { c.ear.loadedMb = Math.max(c.ear.loadedMb || 0, Math.floor(Number(d.mb) || 0)); if (d.progress === undefined) _readyUpdate(); }
